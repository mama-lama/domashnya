    ...
    <table class="table">
      ...
      <tbody id="menuTableBody">
        ...
        <tr class="menu-item-row" data-search="{{ mb_strtolower($item->name . ' ' . $item->description) }}">
          ...
          <td>
            @if($item->is_featured)
              <span class="badge badge-success" style="background-color: #dcfce7; color: #15803d;">Да</span>
            @else
              <span style="color: var(--admin-text-muted); font-size: 13px;">—</span>
            @endif
          </td>
        ...
        @empty
        <tr>
          <td colspan="8" style="text-align: center; padding: 40px 0; color: var(--admin-text-muted);">В меню пока нет блюд. Добавьте первое блюдо!</td>
        </tr>
        @endforelse
      </tbody>
    </table>
