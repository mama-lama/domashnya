  ...
  <form method="POST" action="{{ isset($menuItem) ? route('admin.menu.update', $menuItem) : route('admin.menu.store') }}" enctype="multipart/form-data">
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
      ...
      <div class="form-group" style="margin-bottom: 0; align-self: end;">
        <label style="display: flex; align-items: center; gap: 8px; font-weight: 400; cursor: pointer; min-height: 46px; padding: 8px 14px; border-radius: 14px; background: #faf7ef; border: 1px solid var(--admin-border);">
          <input type="hidden" name="is_featured" value="0" />
          <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $menuItem->is_featured ?? false) ? 'checked' : '' }} />
          <span>Отображать на главной странице</span>
        </label>
      </div>
    </div>
